...
char     *const planet_flags[] = {
		...
		"hoth", "p16", "p17", "p18", "p19", "p20", "p21", "p22",
...
char     *const cargo_names[CONTRABAND_MAX] = {
        ...
        "Durasteel",
        "Electronics", "Serind", "Tetrali", "Computer", "Monothelene",
        "Atomic_cells",
        "Ryll", "Transparisteel", "Droid_parts", "Armour", "Delicacies",
        "Alazhi", "MAX-ERROR", "none", "Blasters", "Spice", "Slaves",
        "Explosives"
};
...
char     *const ex_flags[] = {
        ...
        "bashed", "bashproof", "nomob", "window", "can_look", "rubble"
};
...
char     *const w_flags[] = {
        ...
        "ears", "eyes",
        "_missile_", "binding", "holster", "_holster2_", "r3", "r4", "r5",
        "r6", "r7", "r8", "r9", "r10", "r11", "r12", "r13"
};

char     *const area_flags[] = {
        "nopkill", "noquest", "prototype", "r3", "r4", "r5", "r6", "r7", "r8",
...
char     *const o_types[] = {
        ...
        "scraps", "_unused1", "_unused2", "_unused3", "_incense", "fire",
        "book",
        "switch",
        "lever", "_pullchain", "button", "beacon", "trap", "_runepouch",
        "_match", "raw_metal",
        "canister", "_portal", "paper", "_tinder", "lockpick", "_spike",
        "_disease", "_oil",
        "fuel", "restraint", "_longbow", "_crossbow", "ammo", "_quiver",
        "shovel",
        ...
        "spacecraft",
        "grenade", "landmine", "government", "droid_corpse", "bolt", "bond",
        "implant",
        "chemical", "binding", "holster", "landdeed", "<error>"
};
...
char     *const a_flags[] = {
        ...
        "curse",
        "secretive", "poison", "protect", "paralysis", "sneak", "hide",
        "sleep",
        "charm", "flying", "pass_door", "floating", "truesight",
        "detect_traps",
        "scrying", "fireshield", "shockshield", "restrained", "iceshield",
        "possess",
        "berserk", "aqua_breath"
};

char     *const act_flags[] = {
        "npc", "sentinel", "scavenger", "norunsnipe", "r3", "aggressive",
        "stayarea",
        "wimpy", "pet", "train", "practice", "immortal", "deadly",
        "speaksall",
        "meta_aggr", "guardian", "running", "nowander", "mountable",
        "mounted", "scholar",
        "secretive", "polymorphed", "mobinvis", "noassist", "nokill", "droid",
        "nocorpse",
        "mail", "citizen", "prototype", "r31"
};

char     *const pc_flags[] = {
        "r1", "nohunger", "unauthed", "norecall", "nointro", "gag", "retired",
        "guest",
        "nosummon", "pageron", "notitled", "room", "working", "gotmail",
        "exempt", "fastengineer", "watch", "married",
        "newbieguide", "autodraw", "r20", "r21", "r22",
        "r23", "r24", "r25", "r26", "r27",
        "r28", "r29", "r30", "r31"
};

char     *const plr_flags[] = {
        "npc", "boughtpet", "shovedrag", "autoexits", "autoloot", "forsaken",
        "blank",
        "secretive", "brief", "combine", "prompt", "telnet_ga", "holylight",
        "wizinvis", "roomvnum", "silence", "noemote", "attacker", "notell",
        "log",
        "deny", "freeze", "killer", "pf_3", "litterbug", "ansi", "rip",
        "nice",
        "flee", "autocred", "automap", "afk"
};

char     *const trap_flags[] = {
        "room", "obj", "enter", "leave", "open", "close", "get", "put",
        "pick",
...
char     *const trig_flags[] = {
        ...
        "mload",
        "r16", "r17", "r18", "death", "cast", "fakeblade",
        "rand4", "rand6", "trapdoor", "anotherroom", "usedial",
        "absolutevnum",
...
char     *const part_flags[] = {
        ...
        "fingers",
        "ear", "eye", "long_tongue", "eyestalks", "tentacles", "fins",
        "wings",
...
char     *const mprog_flags[] = {
        "act", "speech", "rand", "fight", "death", "hitprcnt", "entry",
        "greet",
...
{
        unsigned int x;
        unsigned int max = NUMITEMS(body_parts);
...
int get_risflag(char *flag)
{
        ...
        for (x = 0; x < NUMITEMS(ris_flags); x++)
...
int get_partflag(char *flag)
{
        ...
        for (x = 0; x < (int) NUMITEMS(part_flags); x++)
...
CMDF do_goto(CHAR_DATA * ch, char *argument)
{
        ...
        if (!IS_SET(ch->act, PLR_WIZINVIS))
        {
                if (ch->pcdata && ch->pcdata->bamfout[0] != '\0')
                        ...
                        act(AT_IMMORT, "$n $T", ch, NULL,
                            (void *) "leaves in a swirl of the force.",
                            TO_ROOM);
        }
        ...
        if (!IS_SET(ch->act, PLR_WIZINVIS))
        {
                if (ch->pcdata && ch->pcdata->bamfin[0] != '\0')
                        ...
                        act(AT_IMMORT, "$n $T", ch, NULL,
                            (void *) "enters in a swirl of the Force.",
                            TO_ROOM);
        }
...
CMDF do_mset(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg1[0] == '\0' || (arg2[0] == '\0') || !str_cmp(arg1, "?"))
        {
                ...
                send_to_char("  speaking (see LANGUAGES)\n\r", ch);
                send_to_char
        ...
        {
                if (ch->top_level < sysdata.level_mset_player)
                {
                        pager_printf(ch,
                                     "You need to be at least level %d to mset players.\n\r",
        ...
        else if (!str_cmp(arg2, "wanted"))
        {
                send_to_char("Use pardon instead.\n\r", ch);
                return;
        }
        ...
        if (!str_cmp(arg2, "home"))
        {
                ...
                if (!str_cmp("none", arg3))
                {
                        ...
                        send_to_char("Victims player home set to none!", ch);
                        return;
                }

                victim->plr_home = get_room_index(value);
                if (victim->plr_home == NULL)
                        send_to_char("Victims player not set, bad vnum!", ch);
                return;
        }
        ...
        if (!str_cmp(arg2, "implant"))
        {
                ...
                if (value < 0 || value >= 7)
                {
                        send_to_char("Implant range between 0 and 7\n\r", ch);
                        return;
                }
        ...
        if (!str_cmp(arg2, "held"))
        {
                ...
                {
                        victim->held = victim->held ? FALSE : TRUE;
                        if (victim->held)
                                send_to_char("Held state set.\n\r", ch);
                        else if (!victim->held)
                                send_to_char("Held state removed.\n\r", ch);
                        return;
                }
        }
        if (!str_cmp(arg2, "resetheld"))
        {
                ...
                else
                {
                        ...
                        victim->heldby = NULL;
                        victim->holding = NULL;
                        send_to_char("All held structures cleared.\n\r", ch);
                        return;
                }
        }
        if (!str_cmp(arg2, "mainability"))
        {
                int       iclass;
        ...
        if (!str_cmp(arg2, "race"))
        {
                if (!can_mmodify(ch, victim))
                        return;
                victim->race = get_race(arg3);
                if (victim->race == NULL)
                {
                        victim->race = get_race("human");
        ...
        if (!str_cmp(arg2, "hp"))
        {
                ...
                {
                        send_to_char
                                ("Hp range is 1 to 32,700 hit points.\n\r",
                                 ch);
                        return;
                }
        ...
        if (!str_cmp(arg2, "quest"))
        {
                ...
                {
                        send_to_char
                                ("The current quest range is 0 to 10000.\n\r",
                                 ch);
                        return;
                }
        ...
        if (!str_cmp(arg2, "qp"))
        {
                ...
                {
                        send_to_char
                                ("The current quest point range is 0 to 10000.\n\r",
                                 ch);
                        return;
                }
        ...
        if (!str_cmp(arg2, "spec") || !str_cmp(arg2, "spec_fun"))
        {
                ...
                if (!arg3 || arg3[0] == '\0')
                {
                        ...
                        send_to_char
                                ("Please select one of the following special functions:\n\r",
                                 ch);
                        {
                                for (specfun = first_specfun; specfun;
                                     specfun = specfun->next, count++)
                                {
                                        ch_printf(ch, "%s\t\t",
                                                  specfun->name);
                                        if ((count % 3) == 0)
        ...
        if (!str_cmp(arg2, "spec2") || !str_cmp(arg2, "spec2_fun"))
        {
                ...
                if (IS_NPC(victim) && IS_SET(victim->act, ACT_PROTOTYPE))
                {
                        ...
                        STRFREE(victim->pIndexData->spec2_funname);
                        victim->pIndexData->spec2_funname = STRALLOC(arg3);
                }
                send_to_char("Victim special function set.\n\r", ch);
                return;
        }
                        ...
                        else
                        {
                                ...
                                mudstrlcat(buf,
                                           show_ext_flag_string(NUMITEMS
                                                                ...
                                                                pc_flags),
                                           MSL);
                                send_to_char(wordwrap(buf, 78), ch);
                        }
                        send_to_char("\n\r", ch);
                        return;
                }
        ...
        if (!str_cmp(arg2, "godflags"))
        {
                ...
                {
                        send_to_char
                                ("Usage: mset player godflags flags <flag> [flag]...\n\r",
                                 ch);
                        send_to_char
                ...
                while (argument[0] != '\0')
                {
                        ...
                        TOGGLE_BIT(victim->pcdata->godflags, 1 << tempnum);
                }
                send_to_char("Done.\n\r", ch);
                return;
        }
        ...
        if (!str_cmp(arg2, "damsizedie"))
        {
                ...
                {
                        send_to_char
                                ("Damage dice size range is 0 to 100.\n\r",
                                 ch);
                        return;
                }
                if (IS_NPC(victim) && IS_SET(victim->act, ACT_PROTOTYPE))
                ...
                if (!argument || argument[0] == '\0')
                {
                        ...
                        send_to_char
                                ("Usage: mset <victim> bounty <number> <field> <value>\n\r",
                                 ch);
                        send_to_char
                                ("       Field being one of: vnum amount experience type\n\r",
                                 ch);
                        return;
                }
                mset_bounty(ch, victim, argument);
                send_to_char("Done.\n\r", ch);
                return;
        }
...
CMDF do_oset(CHAR_DATA * ch, char *argument)
{
        ...
        if (!str_cmp(arg2, "ed"))
        {
                ...
                {
                        send_to_char
                                ("Syntax: oset <object> ed <keywords>\n\r",
                                 ch);
                        return;
                }
 ...
 * GAVIN - FIXME - should return an error code somehow, and use the macro names not magic numbers
 */
...
char     *sprint_reset(CHAR_DATA * ch, RESET_DATA * pReset, sh_int num,
                       bool rlist)
{
        static char buf[MAX_STRING_LENGTH];
        ...
        case 'D':
                ...
                {
                        mudstrlcpy(roomname, "Room: *BAD VNUM*", MSL);
                        snprintf(objname, MSL, "%s (no exit)",
                                 dir_name[pReset->arg2]);
                }
                else
                {
                        mudstrlcpy(roomname, room->name, MSL);
                        snprintf(objname, MSL, "%s%s",
                                 dir_name[pReset->arg2],
                                 get_exit(room,
                                          pReset->arg2) ? "" : " (NO EXIT!)");
                }
...
CMDF do_redit(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg[0] == '\0' || !str_cmp(arg, "?"))
        {
                ...
                send_to_char("  exit bexit exdesc exflags exname exkey\n\r",
                             ch);
                send_to_char("  flags sector rlist ex_to_room\n\r", ch);
                send_to_char("  tunnel bexflags\n\r", ch);
                return;
        }
        ...
        if (!str_cmp(arg, "rlist"))
        {
                ...
                num = 0;
                for (pReset = tarea->first_reset; pReset;
                     pReset = pReset->next)
                {
                        (num)++;
                        if ((bptr =
        ...
        if (!str_cmp(arg, "exkey"))
        {
                ...
                {
                        send_to_char
                                ("Usage: redit exkey <dir> <key vnum>\n\r",
                                 ch);
                        return;
                }
                if (arg2[0] == '#')
        ...
        if (!str_cmp(arg, "exname"))
        {
                ...
                if (arg2[0] == '\0')
                {
                        send_to_char("Change or clear exit keywords.\n\r",
                                     ch);
                        send_to_char
                                ("Usage: redit exname <dir> [keywords]\n\r",
                                 ch);
                        return;
                }
                if (arg2[0] == '#')
        ...
        if (!str_cmp(arg, "bexflags"))
        {
                if (!argument || argument[0] == '\0')
                {
                        send_to_char("Toggle or display exit flags.\n\r", ch);
                        send_to_char
                                ("Usage: redit bexflags <dir> <flag> [flag]...\n\r",
                                 ch);
                        send_to_char("\n\rExit flags:\n\r", ch);
                        send_to_char(wordwrap(show_ext_flag_string
                                              (NUMITEMS(ex_flags), ex_flags),
        ...
        if (!str_cmp(arg, "exit"))
        {
                ...
                if (!str_cmp(argument, "remove all"))
                {
                        EXIT_DATA *xit = NULL;
                ...
                {
                        send_to_char
                                ("You can't make an exit to that room.\n\r",
                                 ch);
                        return;
                }
                if (addexit || !xit)
...
void free_area(AREA_DATA * are)
{
        ...
        for (icnt = 0; icnt < MAX_KEY_HASH; icnt++)
        {
                ...
                for (mid = mob_index_hash[icnt]; mid; mid = mid_next)
                {
                        ...
                        if (mid->vnum < are->low_m_vnum
                            || mid->vnum > are->hi_m_vnum)
                                continue;
                        delete_mob(mid);
                }
                ...
                for (oid = obj_index_hash[icnt]; oid; oid = oid_next)
                {
                        ...
                        if (oid->vnum < are->low_o_vnum
                            || oid->vnum > are->hi_o_vnum)
                                continue;
                        delete_obj(oid);
                }
        }
...
void fold_area(AREA_DATA * tarea, char *filename, bool install, bool dolog)
{
        ...
        for (vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++)
        {
                ...
                if (!pMobIndex->race || !pMobIndex->race->name())
                        pMobIndex->race = get_race("human");
                fprintf(fpout, "%s~\n", pMobIndex->race->name());
                if (pMobIndex->speaking && pMobIndex->speaking->name)
                        fprintf(fpout, "%s~\n", pMobIndex->speaking->name);
                else
                        fprintf(fpout, "%s~\n",
                                pMobIndex->race->language()->name);
                ...
                if (!pMobIndex->max_hit)
                        fprintf(fpout, "%d\n",
                                pMobIndex->hitnodice ? pMobIndex->hitnodice *
                                number_range(1,
                                             pMobIndex->hitsizedice) +
                                pMobIndex->hitplus : pMobIndex->level * 10 +
                                number_range(pMobIndex->level,
                                             pMobIndex->level * 10));
                else
                        fprintf(fpout, "%d\n", pMobIndex->max_hit);
                fprintf(fpout, "%d 0\n", pMobIndex->gold);
...
CMDF do_loadarea(CHAR_DATA * ch, char *argument)
{
        AREA_DATA *tarea;
        char      filename[256];
        int       tmp;
...
CMDF do_installarea(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Syntax: installarea <filename> [Area title]\n\r",
                         ch);
                return;
        }
        ...
        for (tarea = first_build; tarea; tarea = tarea->next)
        {
                if (!str_cmp(tarea->filename, arg))
                {
                         ...
                         */
                        UNLINK(tarea, first_bsort, last_bsort, next_sort,
                               prev_sort);
...
CMDF do_astat(CHAR_DATA * ch, char *argument)
{
        ...
        ch_printf(ch, "Planet: %s\n\r",
                  tarea->planet ? tarea->planet->name : "none");
        ...
        if (tarea->first_person)
        {
                send_to_char("Players in area:\n\r", ch);
                for (vch = tarea->first_person; vch; vch = vch->next_in_area)
                {
                        ch_printf(ch, "  &G%s&w\n\r", vch->name);
                }
        }
...
CMDF do_aset(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Usage: aset <area filename> <field> <value>\n\r",
                         ch);
                send_to_char("\n\rField being one of:\n\r", ch);
                send_to_char
        ...
        if (!str_cmp(arg2, "planet"))
        {
                ...
                {
                        planet = tarea->planet;
                        UNLINK(tarea, planet->first_area, planet->last_area,
                               next_on_planet, prev_on_planet);
                        tarea->planet = NULL;
                        save_planet(planet, FALSE);
                }
                ...
                {
                        send_to_char("No such planet", ch);
                        return;
                }
                return;
        }
...
CMDF do_opedit(CHAR_DATA * ch, char *argument)
{
        ...
        else
        {
                ...
                {
                        send_to_char
                                ("Nothing like that in all the realms.\n\r",
                                 ch);
                        return;
                }
        }
...
CMDF do_rpedit(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg1[0] == '\0')
        {
                ...
                send_to_char("  add delete insert edit list\n\r", ch);
                send_to_char("Program being one of:\n\r", ch);
                send_to_char("  act speech rand sleep rest rfight greet\n\r",
                             ch);
                send_to_char("  leave death allgreet\n\r", ch);
                send_to_char("\n\r", ch);
                send_to_char
...
CMDF do_rdelete(CHAR_DATA * ch, char *argument)
{
         ...
         * Find the room. 
         */
        if ((roomnum = get_room_index(room)) == NULL)
        {
                send_to_char("No such location.\n\r", ch);
                return;
        }
        ...
        for (obj = roomnum->first_content; obj; obj = obj_next)
        {
                        obj_next = obj->next_content;
                        extract_obj(obj);
        }

        if (roomnum->first_person || roomnum->first_content)
        {
...
CMDF do_ropen(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg1[0] == '\0' || arg2[0] == '\0')
        {
                send_to_char("Syntax: ropen <start> <end>\n\r", ch);
                return;
        }
...
CMDF do_testtest(CHAR_DATA * ch, char *argument)
{
        ...
        argument = one_argument(argument, arg);
        snprintf(argument, 255, "%s %s", argument, "moo");
        ch_printf(ch, "Arg: %s Argument: %s\n\r", arg, argument);
}
