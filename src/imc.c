...
char     *color_mtoi(const char *txt)
{
        ...
        for (color = first_imc_color; color; color = color->next)
                imcstrlcpy(tbuf,
                           imcstrrep(tbuf, color->mudtag, color->imctag),
                           LGST);
...
void imc_new_reminfo(char *mud, char *version, char *netname, char *url,
                     char *path)
{
                          ...
                          bool enforceequal)
{
        ...
        {
                imc_to_char
                        ("You cannot set permissions higher than your own.\n\r",
                         ch);
                return FALSE;
        }
...
void imcformat_channel(CHAR_DATA * ch, IMC_CHANNEL * d, int format, bool all)
{
        ...
        if (all)
        {
                for (c = first_imc_channel; c; c = c->next)
                {
                        ...
                        {
                                snprintf(buf, LGST, "~R[~Y%s~R] ~c%%s %%s",
                                         c->local_name);
                                IMCSTRFREE(c->emoteformat);
        ...
        else
        {
                ...
                {
                        imc_to_char
                                ("This channel is not yet locally configured.\n\r",
                                 ch);
                        return;
                }
...
void imc_new_channel(char *chan, char *owner, char *ops, char *invite,
                     char *exclude, bool copen, int perm, char *lname)
{
        IMC_CHANNEL *c;

        if (!chan || chan[0] == '\0')
        {
                imclog("%s: NULL channel name received, skipping",
                       __FUNCTION__);
                return;
        }
        ...
        if (!strchr(chan, ':'))
        {
                imclog("%s: Improperly formatted channel name: %s",
                       __FUNCTION__, chan);
                return;
        }
...
char     *imcfread_line(FILE * fp)
{
        ...
        do
        {
                ...
                {
                        imcbug("%s",
                               "imcfread_line: EOF encountered on read.");
...
void imc_write_buffer(const char *txt)
{
        ...
        {
                imcbug("%s: Configuration or socket is invalid!",
                       __FUNCTION__);
                return;
        }
...
void imc_write_packet(IMC_PACKET * p)
{
        ...
        snprintf(txt, IMC_BUFF_SIZE, "%s %lu %s %s %s", p->from,
                 ++imc_sequencenumber, this_imcmud->localname, p->type,
                 p->to);
...
PFUN(imc_recv_tell)
{
        ...
        if (strcasecmp(imc_nameof(q->from), "ICE"))
        {
                ...
                if (IMCAFK(vic))
                {
                        ...
                        {
                                snprintf(buf, LGST,
                                         "%s is currently AFK. Try back later.",
                                         q->to);
                                imc_send_tell("*", q->from, buf, 1);
                        }
                        return;
                ...
                if (strcasecmp(imc_nameof(q->from), "*"))
                {
                        ...
                        IMC_RREPLY_NAME(vic) =
                                IMCSTRALLOC(imcgetname(vic, q->from));
                }
        }
        /*
         * Tell social 
        ...
        for (x = 0; x < MAX_IMCHISTORY; x++)
        {
                if (channel->history[x] == NULL)
                {
                        ...
                        if (IMCIS_SET(channel->flags, IMCCHAN_LOG))
                        {
                                ...
                                {
                                        perror(buf);
                                        imcbug("Could not open file %s!",
                                               buf);
                                }
                                else
                                {
                                        fprintf(fp, "%s\n",
                                                imc_strip_colors(channel->
                                                                 history[x]));
                                        IMCFCLOSE(fp);
                                }
                        }
                         ...
                         int emote)
{
        ...
        if (emote < 2)
                snprintf(buf, LGST, emote ? c->emoteformat : c->regformat,
                         from, txt);
        else
...
PFUN(imc_recv_channelnotify)
{
        ...
        if (chon == TRUE)
                snprintf(buf, LGST, c->emoteformat, q->from,
...
void imc_process_who(char *from)
{
        DESCRIPTOR_DATA *d;
        CHAR_DATA *person;
        char      buf[LGST], whobuf[LGST], personbuf[LGST], tailbuf[LGST],
                rank[LGST], rankout[LGST];
        char      stats[SMST];
        char      whoreply[IMC_BUFF_SIZE];
        int       pcount = 0, xx, yy;

        imcstrlcpy(whoreply, "\n\r", IMC_BUFF_SIZE);
        snprintf(whobuf, LGST, "~R-=[ ~WPlayers on %s ~R]=-",
        ...
        if (this_imcmud->iport > 0)
                snprintf(whobuf, LGST, "~Y-=[ ~Wtelnet://%s:%d ~Y]=-",
                         this_imcmud->ihost, this_imcmud->iport);
        else
                snprintf(whobuf, LGST, "~Y-=[ telnet://%s ]=-",
                         this_imcmud->ihost);
        ...
        snprintf(tailbuf, LGST, "\n\r~Y[~WHomepage: %s~Y] [~W%d Player%s~Y] ",
                 this_imcmud->www, pcount, pcount == 1 ? "" : "s");
...
void imc_process_finger(char *from, char *type)
{
        ...
        if (!IMCIS_SET(IMCFLAG(victim), IMC_PRIVACY))
                snprintf(buf + strlen(buf), IMC_BUFF_SIZE - strlen(buf),
                         ...
                         (IMC_EMAIL(victim)
                          && IMC_EMAIL(victim)[0] !=
                          '\0') ? IMC_EMAIL(victim) : "None",
                         (IMC_HOMEPAGE(victim)
                          && IMC_HOMEPAGE(victim)[0] !=
                          '\0') ? IMC_HOMEPAGE(victim) : "None",
                         IMC_ICQ(victim), (IMC_AIM(victim)
                                           && IMC_AIM(victim)[0] !=
                                           '\0') ? IMC_AIM(victim) : "None",
                         (IMC_YAHOO(victim)
                          && IMC_YAHOO(victim)[0] !=
...
PFUN(imc_recv_beep)
{
        ...
        if (IMCISINVIS(vic))
        {
                if (strcasecmp(imc_nameof(q->from), "*"))
                {
                        snprintf(buf, LGST, "%s is not receiving beeps.",
                                 q->to);
                        imc_send_tell("*", q->from, buf, 1);
                }
                return;
...
{
        IMC_CHANNEL *c;
        char      chan[SMST], owner[SMST], ops[SMST], invite[SMST],
                exclude[SMST], policy[SMST], level[SMST], lname[SMST];
        int       perm;
        bool      copen;
        ...
        if (!(c = imc_findchannel(chan)))
        {
                imc_new_channel(chan, owner, ops, invite, exclude, copen,
                                perm, lname);
                return;
        }
...
void imc_prune_ucache(void)
{
        ...
        for (ucache = first_imcucache; ucache; ucache = next_ucache)
        {
                ...
                if (imc_time - ucache->time >= 2592000)
                {
                        ...
                        IMCUNLINK(ucache, first_imcucache, last_imcucache,
                                  next, prev);
                        IMCDISPOSE(ucache);
                }
        }
...
void imc_handle_autosetup(char *source, char *routername, char *cmd,
                          char *txt, char *md5)
{
        if (!strcasecmp(cmd, "reject"))
        {
                ...
                {
                        imclog("%s is a private router. Autosetup denied.",
                               routername);
                        imc_shutdown(FALSE);
                        return;
                }
                ...
                imclog("%s: Invalid 'reject' response. Autosetup failed.",
                       routername);
                imclog("Data received: %s %s %s %s %s", source, routername,
                       cmd, txt, md5);
                imc_shutdown(FALSE);
                return;
        }
        ...
        imclog("Data received: %s %s %s %s %s", source, routername, cmd, txt,
               md5);
        imc_shutdown(FALSE);
        return;
}
...
bool imc_write_socket(void)
{
        ...
        while (nleft > 0)
        {
                if ((nwritten = send(this_imcmud->desc, ptr, nleft, 0)) <= 0)
                {
                        ...
                        if (nwritten < 0)
                                imclog("Write error on socket: %s",
                                       strerror(errno));
                        else
                                imclog("%s",
                                       "Connection close detected on socket write.");
...
void imc_process_authentication(char *packet)
{
        ...
        if (!strcasecmp(command, "MD5-AUTH-INIT"))
        {
                 ...
                 */
                auth_value = atol(pw);
                snprintf(pwd, SMST, "%ld%s%s", auth_value,
...
bool imc_read_socket(void)
{
        ...
        for (;;)
        {
                ...
                {
                        imclog("%s: Descriptor error on #%d: %s",
                               __FUNCTION__, this_imcmud->desc,
                               strerror(iErr));
                        return FALSE;
                }
        }
...
bool imc_loadchar(CHAR_DATA * ch, FILE * fp, const char *word)
{
        ...
        case 'I':
                            ...
                            && this_imcmud->state == IMC_ONLINE)
                        {
                                ...
                                while (1)
                                {
                                        if (channels[0] == '\0')
                                                break;
                                        channels =
                                                imcone_argument(channels,
                                                                arg);

                                        if (!(channel = imc_findchannel(arg)))
                                                imc_removename(&IMC_LISTEN
                                                               (ch), arg);
                                        if (channel
                ...
                if (!strcasecmp(word, "IMCignore"))
                {
                        ...
                        IMCLINK(temp, FIRST_IMCIGNORE(ch), LAST_IMCIGNORE(ch),
                                next, prev);
                        fMatch = TRUE;
                        break;
                }
...
void imc_loadhistory(void)
{
        ...
        for (tempchan = first_imc_channel; tempchan;
...
void imc_readchannel(IMC_CHANNEL * channel, FILE * fp)
{
        ...
        for (;;)
        {
                ...
                case 'C':
                        ...
                        IMCKEY("ChanLocal", channel->local_name,
                               imcfread_line(fp));
                        IMCKEY("ChanRegF", channel->regformat,
                               imcfread_line(fp));
                        IMCKEY("ChanEmoF", channel->emoteformat,
                               imcfread_line(fp));
                        IMCKEY("ChanSocF", channel->socformat,
                               imcfread_line(fp));
                        IMCKEY("ChanLevel", channel->level,
                               imcfread_number(fp));
                        break;
...
void imc_savebans(void)
{
        ...
        {
                imcbug("%s",
                       "imc_savebans: error opening ban file for write");
                return;
        }
...
void imc_load_color_table(void)
{
        ...
        for (;;)
        {
                ...
                if (!strcasecmp(word, "COLOR"))
                {
                        ...
                        IMCLINK(color, first_imc_color, last_imc_color, next,
                                prev);
                        continue;
                }
                else if (!strcasecmp(word, "END"))
                ...
                {
                        imcbug("imc_load_color_table: bad section: %s.",
                               word);
                        continue;
                }
        }
...
void imc_load_helps(void)
{
        ...
        for (;;)
        {
                ...
                if (!strcasecmp(word, "HELP"))
                {
                        ...
                        IMCLINK(help, first_imc_help, last_imc_help, next,
                                prev);
                        continue;
                }
                else if (!strcasecmp(word, "END"))
        ...
        for (;;)
        {
                ...
                case 'A':
                        if (!strcasecmp(word, "Alias"))
                        {
                                ...
                                IMCLINK(alias, cmd->first_alias,
                                        cmd->last_alias, next, prev);
                                fMatch = TRUE;
                                break;
                        }
                        break;
                ...
                case 'C':
                        IMCKEY("Connected", cmd->connected,
                               imcfread_number(fp));
...
bool imc_load_commands(void)
{
        ...
        for (;;)
        {
                ...
                if (!strcasecmp(word, "COMMAND"))
                {
                        ...
                        IMCLINK(cmd, first_imc_command, last_imc_command,
                                next, prev);
                        continue;
                }
                else if (!strcasecmp(word, "END"))
...
void imc_load_ucache(void)
{
        ...
        for (;;)
        {
                ...
                if (!strcasecmp(word, "UCACHE"))
                {
                        ...
                        IMCLINK(user, first_imcucache, last_imcucache, next,
                                prev);
                        continue;
                }
                else if (!strcasecmp(word, "END"))
...
void imcfread_config_file(FILE * fin)
{
        ...
        for (;;)
        {
                ...
                case 'C':
                        IMCKEY("ClientPwd", this_imcmud->clientpw,
                               imcfread_line(fin));
                        break;
                ...
                case 'I':
                        IMCKEY("Implevel", this_imcmud->implevel,
                               imcfread_number(fin));
                        IMCKEY("InfoName", this_imcmud->fullname,
                               imcfread_line(fin));
                        IMCKEY("InfoHost", this_imcmud->ihost,
                               imcfread_line(fin));
                        IMCKEY("InfoPort", this_imcmud->iport,
                               imcfread_number(fin));
                        IMCKEY("InfoEmail", this_imcmud->email,
                               imcfread_line(fin));
                        IMCKEY("InfoWWW", this_imcmud->www,
                               imcfread_line(fin));
                        IMCKEY("InfoBase", this_imcmud->base,
                               imcfread_line(fin));
                ...
                case 'M':
                        ...
                        IMCKEY("MD5Pwd", this_imcmud->md5pass,
                               imcfread_number(fin));
                ...
                case 'R':
                        IMCKEY("RouterAddr", this_imcmud->rhost,
                               imcfread_line(fin));
                        IMCKEY("RouterPort", this_imcmud->rport,
                               imcfread_number(fin));
                        break;

                case 'S':
                        IMCKEY("ServerPwd", this_imcmud->serverpw,
                               imcfread_line(fin));
                        break;
                }
                if (!fMatch)
...
bool imc_read_config(int desc)
{
        ...
        {
                imcbug("%s",
                       "imc_read_config: No router connection information!!");
...
int cygwin_connect(void)
{
        ...
        if (!inet_aton(this_imcmud->rhost, &sa.sin_addr))
        {
                ...
                {
                        imclog("%s",
                               "imc_connect_to: Cannot resolve router hostname.");
                        imc_shutdown(FALSE);
                        return -1;
                }
...
bool imc_router_connect(void)
{
        ...
        {
                desc = socket(ai->ai_family, ai->ai_socktype,
                              ai->ai_protocol);
                if (desc < 0)
                        continue;
        ...
        if (this_imcmud->md5)
        {
                ...
                {
                        snprintf(buf, LGST,
                                 "PW %s %s version=%d autosetup %s MD5",
                                 this_imcmud->localname,
                                 this_imcmud->clientpw, IMC_VERSION,
                                 this_imcmud->serverpw);
                }
                /*
                 ...
                 */
                        ...
                        snprintf(buf, LGST, "MD5-AUTH-REQ %s",
                                 this_imcmud->localname);
        }
        /*
...
void free_imcdata(bool complete)
{
        ...
        for (ucache = first_imcucache; ucache; ucache = next_ucache)
        {
                next_ucache = ucache->next;
                IMCSTRFREE(ucache->name);
                IMCUNLINK(ucache, first_imcucache, last_imcucache, next,
                          prev);
                IMCDISPOSE(ucache);
        }
        ...
        if (complete)
        {
                for (cmd = first_imc_command; cmd; cmd = cmd_next)
                {
                        ...
                        for (alias = cmd->first_alias; alias;
                             alias = alias_next)
                        {
                                alias_next = alias->next;

                                IMCSTRFREE(alias->name);
                                IMCUNLINK(alias, cmd->first_alias,
                                          cmd->last_alias, next, prev);
                                IMCDISPOSE(alias);
                        }
                        IMCSTRFREE(cmd->name);
                        IMCUNLINK(cmd, first_imc_command, last_imc_command,
                                  next, prev);
                        IMCDISPOSE(cmd);
                }
                ...
                for (help = first_imc_help; help; help = help_next)
                {
                        ...
                        IMCUNLINK(help, first_imc_help, last_imc_help, next,
                                  prev);
                        IMCDISPOSE(help);
                }
                ...
                for (color = first_imc_color; color; color = color_next)
                {
                        ...
                        IMCUNLINK(color, first_imc_color, last_imc_color,
                                  next, prev);
                        IMCDISPOSE(color);
                }
                ...
                for (ph = first_phandler; ph; ph = ph_next)
                {
                        ...
                        IMCSTRFREE(ph->name);
                        IMCUNLINK(ph, first_phandler, last_phandler, next,
                                  prev);
                        IMCDISPOSE(ph);
                }
        }
                ...
                else
                {
                        fprintf(fp, "%s %s\n",
                                (this_imcmud->network ? this_imcmud->
                                 network : "Unknown"),
                                (this_imcmud->routername ? this_imcmud->
                                 routername : "Unknown"));
                        IMCFCLOSE(fp);
                        imc_savehistory();
                }
...
void imc_shutdown(bool reconnect)
{
        ...
        if (reconnect)
        {
                ...
                imclog("%s",
                       "Connection to router was lost. Reconnecting in approximately 20 seconds.");
        }
}
...
bool imc_startup_network(bool connected)
{
        ...
        if (connected)
        {
                ...
                if (!(fp = fopen(IMC_HOTBOOT_FILE, "r")))
                        imcbug("%s: Unable to load IMC hotboot file.",
                               __FUNCTION__);
                else
                {
...
void imc_startup(bool force, int desc, bool connected)
{
        ...
        if (first_imc_command == NULL)
        {
                ...
                {
                        imcbug("%s: Unable to load command table!",
                               __FUNCTION__);
                        return;
                }
        }
...
IMC_CMD(imccommand)
{
        ...
        {
                imc_to_char
                        ("Syntax: imccommand <command> <router:channel> [<data..>]\n\r",
                         ch);
                imc_to_char
                        ("Command access will depend on your privledges and what each individual router allows.\n\r",
                         ch);
                return;
        }
...
IMC_CMD(imcsetup)
{
        ...
        {
                imc_to_char
                        ("Syntax: imcsetup <command> <channel> [<data..>]\n\r",
                         ch);
        ...
        if (!strcasecmp(imccmd, "delete"))
        {
                ...
                {
                        imc_to_char
                                ("You cannot perform a delete all on channels.\n\r",
                                 ch);
                        return;
                }
                IMCSTRFREE(c->local_name);
        ...
        if (!strcasecmp(imccmd, "rename"))
        {
                ...
                {
                        imc_to_char
                                ("You cannot perform a rename all on channels.\n\r",
                                 ch);
                        return;
                }
        ...
        if (!strcasecmp(imccmd, "regformat"))
        {
                ...
                {
                        imc_to_char
                                ("Bad format - must contain exactly 2 %s's.\n\r",
                                 ch);
                        return;
                }
        ...
        if (!strcasecmp(imccmd, "emoteformat"))
        {
                ...
                if (all)
                {
                        ...
                        imc_to_char
                                ("All channel emote formats have been updated.\n\r",
                                 ch);
                }
                else
                {
        ...
        if (!strcasecmp(imccmd, "socformat"))
        {
                ...
                {
                        imc_to_char
                                ("Bad format - must contain exactly 1 %s.\n\r",
                                 ch);
                        return;
                }
...
IMC_CMD(imcchanlist)
{
        ...
        {
                imc_to_char
                        ("~WThere are no known channels on this network.\n\r",
                         ch);
                return;
        }
        ...
        if (argument && argument[0] != '\0')
        {
                ...
                {
                        imc_printf(ch,
                                   "There is no channel called %s here.\n\r",
                                   argument);
                        return;
                }
        }
...
IMC_CMD(imcreply)
{
        ...
        {
                imc_send_tell(CH_IMCNAME(ch), IMC_RREPLY(ch),
                              color_mtoi(argument), 0);
                snprintf(buf1, LGST, "~cYou imctell ~C%s ~c'~W%s~c'\n\r",
...
IMC_CMD(imclist)
{
        ...
        imcpager_printf(ch, "\n\r\n\r~c%-15.15s ~B%-40.40s ~g%-15.15s ~G%s",
                        ...
                        this_imcmud->network, this_imcmud->routername);
...
IMC_CMD(imcconfig)
{
        ...
        if (!arg1 || arg1[0] == '\0')
        {
                ...
                imc_to_char
                        ("~wInfoname       : ~GName of your mud, as seen from the imcquery info sheet.\n\r",
                         ch);
                imc_to_char
                        ("~wInfohost       : ~GTelnet address of your mud.\n\r",
                         ch);
                imc_to_char
                        ("~wInfoport       : ~GTelnet port of your mud.\n\r",
                         ch);
                imc_to_char
                ...
                imc_to_char
                        ("~wInfoWWW        : ~GThe Web address of your mud.\n\r",
                         ch);
                imc_to_char
                        ("~wInfoBase       : ~GThe codebase your mud uses.\n\r",
                         ch);
                imc_to_char
                        ("~wInfoDetails    : ~GSHORT Description of your mud.\n\r",
                         ch);
                imc_to_char
                        ("~wRouterAddr     : ~GDNS or IP address of the router you mud connects to.\n\r",
                         ch);
                imc_to_char
                        ("~wRouterPort     : ~GPort of the router your mud connects to.\n\r",
                         ch);
                imc_to_char
        ...
        if (!strcasecmp(arg1, "show"))
        {
                ...
                if (this_imcmud->md5)
                        imc_to_char
                                ("~RThis mud has enabled MD5 authentication.\n\r",
                                 ch);
                else
                        imc_to_char
                                ("~RThis mud has disabled MD5 authentication.\n\r",
                                 ch);
                if (this_imcmud->md5 && this_imcmud->md5pass)
                        imc_to_char
                                ("~RThe mud is using MD5 encryption to authenticate.\n\r",
                                 ch);
                else
                        imc_to_char
                ...
                snprintf(buf, SMST,
                         "~GThe mud's IMC2 connections is currently connected through descriptor %d.\n\r",
                         this_imcmud->desc);
                imc_to_char(buf, ch);
                return;
        }
...
IMC_CMD(imcignore)
{
        ...
        {
                imc_to_char
                        ("~wTo see your current ignores  : ~GIMCIGNORE\n\r",
                         ch);
                imc_to_char
        ...
        if (!strcasecmp(arg, "delete"))
        {
                for (ign = FIRST_IMCIGNORE(ch); ign; ign = ign->next)
                {
                        ...
                        {
                                IMCUNLINK(ign, FIRST_IMCIGNORE(ch),
                                          LAST_IMCIGNORE(ch), next, prev);
                                IMCSTRFREE(ign->name);
                                IMCDISPOSE(ign);
        ...
        if (!strcasecmp(arg, "add"))
        {
                ...
                IMCLINK(ign, FIRST_IMCIGNORE(ch), LAST_IMCIGNORE(ch), next,
                        prev);
...
IMC_CMD(imcban)
{
        ...
        if (!arg || arg[0] == '\0')
        {
                ...
                for (count = 0, ban = first_imc_ban; ban;
                     ban = ban->next, count++)
                        imc_printf(ch, "%s\n\r", ban->name);
        ...
        {
                imc_to_char
                        ("~wTo see the current bans             : ~GIMCBAN\n\r",
                         ch);
                imc_to_char
...
IMC_CMD(imc_deny_channel)
{
        ...
        if (!vic_name || vic_name[0] == '\0' || !argument
            || argument[0] == '\0')
        {
                imc_to_char
                        ("Usage: imcdeny <person> <local channel name>\n\r",
                         ch);
...
IMC_CMD(imcpermset)
{
        ...
        else
        {
                ...
                if (!imccheck_permissions
                    (ch, permvalue, IMCPERM(victim), TRUE))
                        return;
        }
...
IMC_CMD(imcremoteadmin)
{
        ...
        {
                imc_to_char
                        ("Syntax: imcadmin <router> <password> <command> [<data..>]\n\r",
                         ch);
                imc_to_char
                        ("You must be an approved router administrator to use remote commands.\n\r",
                         ch);
                return;
        }
...
IMC_CMD(imccedit)
{
        ...
        if (!strcasecmp(option, "create"))
        {
                ...
                {
                        imc_printf(ch,
                                   "~gA command named ~W%s ~galready exists.\n\r",
                                   name);
                        return;
                }
        ...
        if (!strcasecmp(option, "delete"))
        {
                ...
                for (alias = cmd->first_alias; alias; alias = alias_next)
                {
                        ...
                        IMCUNLINK(alias, cmd->first_alias, cmd->last_alias,
                                  next, prev);
                        IMCSTRFREE(alias->name);
                        IMCDISPOSE(alias);
                }
        ...
        if (!strcasecmp(option, "alias"))
        {
                ...
                for (tmp = first_imc_command; tmp; tmp = tmp->next)
                {
                        ...
                        {
                                imc_printf(ch,
                                           "~W%s &gis already a command name.\n\r",
                                           argument);
                                return;
                        }
                        for (alias = tmp->first_alias; alias;
                             alias = alias->next)
                        {
                ...
                imc_printf(ch,
                           "~W%s ~ghas been added as an alias for ~W%s\n\r",
                           alias->name, cmd->name);
                imc_savecommands();
                return;
        }
        ...
        {
                imc_printf(ch,
                           "~gCommand ~W%s ~ghas been renamed to ~W%s.\n\r",
                           cmd->name, argument);
                IMCSTRFREE(cmd->name);
...
IMC_CMD(imchedit)
{
        ...
        if (!name || name[0] == '\0' || !cmd || cmd[0] == '\0' || !argument
            || argument[0] == '\0')
        {
...
char     *imc_find_social(CHAR_DATA * ch, char *sname, char *person,
                          char *mud, int victim)
{
        ...
        {
                imc_printf(ch,
                           "~YSocial ~W%s~Y does not exist on this mud.\n\r",
                           sname);
                return socname;
        }
        ...
        if (person && person[0] != '\0' && mud && mud[0] != '\0')
        {
                if (person && person[0] != '\0'
                    && !strcasecmp(person, CH_IMCNAME(ch)) && mud
                    && mud[0] != '\0'
                    && !strcasecmp(mud, this_imcmud->localname))
                {
                        ...
                        {
                                imc_printf(ch,
                                           "~YSocial ~W%s~Y: Missing others_auto.\n\r",
                                           sname);
                                return socname;
                        }
                ...
                else
                {
                        if (victim == 0)
                        {
                                ...
                                imcstrlcpy(socname, social->others_found,
                                           LGST);
                        }
                        else if (victim == 1)
                        {
        ...
        else
        {
                ...
                else
                {
                        ...
                        {
                                imc_printf(ch,
                                           "~YSocial ~W%s~Y: Missing char_no_arg.\n\r",
                                           sname);
                                return socname;
                        }
        ...
        if (person && person[0] != '\0' && mud && mud[0] != '\0')
        {
                    ...
                    && !strcasecmp(mud, this_imcmud->localname))
                {
                        ...
                        imcstrlcpy(socname, social_table[cmd].others_auto,
                                   LGST);
                }
                else
                {
        ...
        else
        {
                if (victim == 0 || victim == 1)
                {
                        ...
                        imcstrlcpy(socname, social_table[cmd].others_no_arg,
                                   LGST);
                }
                else
                {
                        ...
                        imcstrlcpy(socname, social_table[cmd].char_no_arg,
                                   LGST);
                }
        }
#endif
...
bool imc_command_hook(CHAR_DATA * ch, char *command, char *argument)
{
        ...
        {
                imcbug("%s",
                       "Ooops. IMC being called with no configuration!");
                return FALSE;
        }
        ...
        {
                imc_printf(ch,
                           "The %s channel has not yet been refreshed by the router.\n\r",
                           c->local_name);
                return TRUE;
        }
