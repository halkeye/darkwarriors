...
bool      oprog_percent_check(CHAR_DATA * mob, CHAR_DATA * actor,
                              OBJ_DATA * obj, void *vo, int type);
void      rprog_percent_check(CHAR_DATA * mob, CHAR_DATA * actor,
                              OBJ_DATA * obj, void *vo, int type);
        ...
        if (chkchar)
        {
                if (!str_cmp(chck, "ismobinvis"))
                {
                        return (IS_NPC(chkchar)
                                && IS_SET(chkchar->act, ACT_MOBINVIS));
                }
                ...
                if (!str_cmp(chck, "isaffected"))
                {
                        ...
                        return IS_AFFECTED(chkchar,
                                           1 << value) ? TRUE : FALSE;
                }
                if (!str_cmp(chck, "iswanted"))
                {
                        if (IS_NPC(chkchar) || !mob->in_room
                            || !mob->in_room->area
                ...
                {
                        if ((IS_NPC(chkchar) && !chkchar->mob_clan)
                            || (!IS_NPC(chkchar) && !chkchar->pcdata->clan))
                                return FALSE;
                        if (IS_NPC(chkchar))
...
void mprog_speech_trigger(char *txt, CHAR_DATA * actor)
{
        ...
        {
                if (IS_NPC(vmob)
                    && (vmob->pIndexData->progtypes & SPEECH_PROG))
                {
                        if (IS_NPC(actor)
