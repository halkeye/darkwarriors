 ...
 *****************************************************************************************
 *                $Id: shops.c 1330 2005-12-05 03:23:24Z halkeye $                *
 ****************************************************************************************/
#include <sys/types.h>
#include <stdio.h>
#include <string.h>
...
CHAR_DATA *find_keeper(CHAR_DATA * ch)
{
         ...
         */
		/*
		 * Making open and close time the same leave it always open- Greven
		 */
...
CMDF do_buy(CHAR_DATA * ch, char *argument)
{
        ...
        else
        {
                ...
                {
                        snprintf(arg, MSL, "$n buys %d $p%s.", noi,
                                 (obj->
                     ...
                     pet = pet->next_in_room)
                {
                        if (IS_SET(pet->act, ACT_PET) && IS_NPC(pet))
                        {
                                ...
                                ch_printf(ch, "[%2d] %8d - %s\n\r",
                                          pet->top_level,
                                          10 * pet->top_level *
                                          pet->top_level, pet->short_descr);
                        }
                }
                if (!found)
                        send_to_char
                                ("Sorry, we're out of pets right now.\n\r",
                                 ch);
                return;
        }
        else
                    ...
                    char *arg, int maxgold, char *fixstr, char *fixstr2)
{
         ...
         */
                  ...
                  strcmp("all", arg) ? cost : 11 * cost / 10) > ch->gold)
        {
                snprintf(buf, MSL,
...
CMDF do_shops(CHAR_DATA * ch, char *argument)
{
        ...
        for (shop = first_shop; shop; shop = shop->next)
                ch_printf(ch,
                          ...
                          shop->buy_type[2], shop->buy_type[3],
                          shop->buy_type[4]);
        return;
}
...
CMDF do_repairshops(CHAR_DATA * ch, char *argument)
{
        ...
        for (repair = first_repair; repair; repair = repair->next)
                ch_printf(ch,
                          ...
                          repair->keeper, repair->profit_fix,
                          repair->shop_type, repair->open_hour,
                          repair->close_hour, repair->fix_type[0],
